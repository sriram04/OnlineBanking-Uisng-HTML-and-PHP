...
<body style="background-image:url('transfer.jpg');background-repeat: no-repeat;background-size:1500px 650px;"
<center>
<form method="POST">
...
if(isset($_POST['save'])){
	if($bal>=$_POST['amount']){
	$sql="select * from createacc where accountnum=".$_POST['tracnum']."";
    $result1 = mysqli_query($conn,$sql);
 	$test1 = mysqli_fetch_array($result1);
 	$bal1=$test1['balance']+$_POST['amount'];
	...
	if (mysqli_query($conn, $sql1)&&mysqli_query($conn, $sql2)&&$test['pin']==$_POST['pin']) {
    echo "<script>alert('Transferred successfully')</script>";
	} else {
    	echo "<script>alert('You have entered wrong details(pin)')</script>";
	}
	}
	else{
		echo "<script>alert('Could not transfer amount as you do not have enough money!!!')</script>";
	}
}
mysqli_close($conn);
 ?>